...
const segmentSizeClassName: Record<SegmentControlSize, string> = {
  ...
  [SegmentControlSize.Lg]: "px-3 py-2.5 text-sm",
};
...
export function SegmentControl<TValue extends string | number | boolean>({
  ...
  const indicatorStyle =
    options.length > 0
      ...
      : undefined;
  const indicatorRadiusClassName =
    options.length <= 1
      ? "rounded-xl"
      : normalizedSelectedIndex === 0
        ? "rounded-l-xl rounded-r-sm"
        : normalizedSelectedIndex === options.length - 1
          ? "rounded-r-xl rounded-l-sm"
          : "rounded-md";

  const controlClassName = [
    "relative flex overflow-hidden rounded-2xl border border-(--input-border) bg-(--glass-bg-input) p-0",
